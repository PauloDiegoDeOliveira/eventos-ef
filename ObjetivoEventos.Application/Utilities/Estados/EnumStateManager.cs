...
namespace ObjetivoEventos.Application.Utilities.Estados
{
    public static class EnumStateManager
    {
        ...
        {
            if (EnumVerifier(enumType, key) && EnumVerifier(enumType, value) && rules.ContainsKey(key.ToString()))
                if (rules[key.ToString()].Contains(value.ToString())) { return true; }
        ...
        {
            return myEnum.GetType() == enumType ? true : false;
        }

        public static bool EnumVerifier(Type enumType, System.Enum enum1, System.Enum enum2)
        {
            return enum1.GetType() == enumType && enum2.GetType() == enumType ? true : false;
        }
    }
}